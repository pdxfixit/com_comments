...
/**
 * @category       Ninja
 * @copyright      Copyright (C) 2007 - 2013 NinjaForge. All rights reserved.
 * @license        GNU GPLv3 <http://www.gnu.org/licenses/gpl.html>
 * @link           http://ninjaforge.com
 */
class CommentsHelperCurl extends JObject {
    ...
    public function info($key = false, $opt = false) {
        if ($key === false) {
            foreach ($this->sessions as $this->key => $session) {
                if (!isset($this->info[$this->key])) {
                    $this->info[$this->key] = array();
                }
